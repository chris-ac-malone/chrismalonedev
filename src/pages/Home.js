...
const Home = () => {
  ...
  return (
    <>
      <div id="home-head" className={`overflow-hidden ${darkMode == "true" ? "dark" : ""}`}>
        <div className="bg-sky-200 dark:bg-slate-800">
          <button className="fixed left-3 top-1 text-2xl dark:text-yellow-300 text-slate-800" onClick={() => toggleDarkMode()}> <FontAwesomeIcon icon={faMoon} /> </button>
          <div className="h-screen w-full table text-center text-4xl font-bold animate-fade-in">
            <h1 className="table-cell w-full text-center align-middle py-10 dark:text-white text-3xl md:text-7xl">Chris Malone<img className="m-auto mt-10 w-40 rounded-full shadow-lg" src={Profile} alt="Chris Malone Web Developer" />
              <div className="flex max-w-xs m-auto pt-4 gap-20">
                <a href="https://www.linkedin.com/in/christopher-malone-898ba9161/" className="flex-1 animate-slide-in"><img className="social-media-icon" src="https://cdn-icons-png.flaticon.com/512/1384/1384014.png" alt="" /></a>
                <a href="https://github.com/chris-ac-malone" className="flex-1 pt-8 animate-slide-in-delay-200"><img className="social-media-icon" src="https://cdn-icons-png.flaticon.com/512/25/25231.png" alt="" /></a>
                <a href="https://codepen.io/Acedoutry" className="flex-1 animate-slide-in-delay-400"><img className="social-media-icon" src={require("../assets/codepen.png")} alt="" /></a>
              </div></h1>
          </div>
          ...
          <div className="mx-auto px-3 grid lg:grid-cols-3 xl:grid-cols-3 grid-cols-1 relative container text-neutral-900 dark:text-neutral-50">
            ...
            <CardBasic bgImage={require('../assets/cm-update-history.png')} href="https://wordpress.org/plugins/cm-update-history/" cardTitle="CM Update History" cardInfo="A WordPress plugin that tracks plugin updates and displays the data to a dashboard widget. Published on the WordPress plugin directory. " />
